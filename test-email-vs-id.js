...
/**
 * Test pour vérifier si le problème vient de l'utilisation de l'email vs l'ID
 */
...
async function testEmailVsId() {
  console.log('🔍 Test Email vs ID\n');
  ...
  try {
    ...
    // 6. Test avec une requête directe à Supabase
    console.log('6. Test avec une requête directe à Supabase...');
    const response4 = await fetch(`${API_BASE_URL}/api/candidates/profile/${encodeURIComponent(TEST_EMAIL)}?force_refresh=true`);
    
    if (!response4.ok) {
      console.error('❌ Erreur récupération avec force_refresh:', response4.status);
      return;
    }
    
    const candidate4 = await response4.json();
    console.log('✅ Profil récupéré avec force_refresh:');
    console.log('   - ID:', candidate4.id);
    console.log('   - Email:', candidate4.email);
    console.log('   - Plan Type:', candidate4.plan_type);
    console.log('   - Updated At:', candidate4.updated_at);

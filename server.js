...
// GET /api/recruiters/me/complete - Récupérer toutes les données du recruteur en une seule requête (OPTIMISÉ)
app.get('/api/recruiters/me/complete', requireRole(['recruiter', 'admin']), async (req, res) => {
  try {
    ...
    if (req.user.email === 'admin@system') {
      return res.json({
        ...
        stats: {
          ...
          total_candidates_contacted: 0,
          total_applications_received: 0,
          remaining_job_posts: 999999,
          remaining_candidate_contacts: 999999
        },
        permissions: {
          can_post_job: true,
          can_contact_candidate: true,
    ...
    const [profile, stats, permissions] = await Promise.all([
      getRecruiterByEmail(req.user.email),
      getRecruiterByEmail(req.user.email).then(recruiter => 
        recruiter ? getRecruiterStats(recruiter.id) : null
      ),
      getRecruiterByEmail(req.user.email).then(recruiter => 
        recruiter ? getRecruiterPermissions(recruiter.id) : null
      )
    ]);
    ...
    logger.info(`✅ [RECRUITER_COMPLETE] Données récupérées en ${loadTime}ms`);
    ...
    logger.error('Erreur lors de la récupération complète du recruteur', { error: error.message });

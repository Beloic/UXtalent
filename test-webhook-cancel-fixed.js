...
/**
 * Test corrigé du webhook d'annulation avec période de 30 jours
 * Ce script envoie le payload au format correct pour Stripe
 */

console.log('🧪 Test corrigé du webhook d\'annulation avec période de 30 jours');
console.log('================================================================');
...
// Simuler l'appel au webhook handler avec le payload en string
...
.then(data => {
  ...
  if (data.includes('succès') || data.includes('success') || response.status === 200) {
...
setTimeout(() => {
  const payloadUpdated = JSON.stringify(testEventUpdated);

...
export default function AdminDashboard() {
  ...
  // Forum states
  const [activeTab, setActiveTab] = useState('candidates'); // candidates, forum
  ...
  const loadForumData = async () => {
    try {
      ...
      const postsResponse = await fetch('http://localhost:3001/api/forum/posts');
      if (postsResponse.ok) {
      ...
      const statsResponse = await fetch('http://localhost:3001/api/forum/stats');
      if (statsResponse.ok) {
  ...
  const deleteForumPost = async (postId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/admin/forum/posts/${postId}`, {
        method: 'DELETE'
      });
  ...
  const deleteForumReply = async (postId, replyId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/admin/forum/posts/${postId}/replies/${replyId}`, {
        method: 'DELETE'
      });
  ...
  const loadPostDetails = async (postId) => {
    try {
      const response = await fetch(`http://localhost:3001/api/forum/posts/${postId}`);
      if (response.ok) {
  ...
  const approveCandidate = async (candidateId) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/candidates/${candidateId}`, {
        method: 'PUT',
        headers: {
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-blue-600">
                ...
                <span className="text-sm font-bold">{forumStats.totalPosts}</span>
              </div>
            </div>
            <div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-emerald-600">
                ...
                <span className="text-sm font-bold">{forumStats.totalReplies}</span>
              </div>
            </div>
            <div>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-purple-600">
                ...
                <span className="text-sm font-bold">{forumStats.totalUsers}</span>
              </div>
            </div>
            <div>
